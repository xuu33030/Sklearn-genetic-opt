...
@pytest.mark.parametrize("error_score", ["warn", "nan", [], {}])
def test_feature_selection_rejects_invalid_error_score(error_score):
    ...
    assert (
        str(excinfo.value) == f"error_score must be numeric or 'raise', got {error_score!r} instead"
    )
